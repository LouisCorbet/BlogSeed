...
const MAX_DELAY_MS = 8000; // plafond 8s
const ATTEMPT_TTL_MS = 10 * 60_000; // mémorisation 10 min

type Attempt = { count: number; last: number };

// Mémoire process-local (OK pour 1 instance; pour du multi-VM: Redis/Upstash).
declare global {
...
function decodeBasicToken(b64: string) {
  // Edge runtime: utiliser atob() (pas de Buffer)
  const raw = atob(b64);
  const i = raw.indexOf(":");
...
export async function middleware(req: NextRequest) {
  ...
  // Échec → incrémente + backoff exponentiel avec jitter
